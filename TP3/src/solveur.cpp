...
std::vector<int> primMST(std::vector<std::vector<int> >& graph, int nbPoints)
{
    ...
    std::vector<int> key(nbPoints, INT_MAX);   // Key values used to pick minimum weight edge in cut
...
void amelioration(Solution &s, Exemplaire &e, bool afficherSol, bool afficherTmp, clock_t debut)
{
    ...
    std::deque<std::pair<int, int>> arcs;
    int mini = 0, maxi = 0;
    std::vector<std::vector<int> > couts = e.getCouts();
    int meilleurCout = INT_MAX;
    ...
    while(1)
    {
        ...
        case PT_DE_VUE: // ce cas n'est pas utilise dans la version actuelle, MAXI est equivalent
            ...
            {
                int cout = sentiers[valide.second][i];
                if (cout > 0)
                {
                    if (arcs.empty())
                        arcs.push_back(std::pair<int, int>(i, cout));
                    else if (cout < arcs[0].second)
                    {
                        arcs.push_front(std::pair<int, int>(i, cout));
